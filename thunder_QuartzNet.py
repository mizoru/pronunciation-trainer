...
def QuartzNet(feat_in: int = 64,
    filters: List[int] = [256, 256, 512, 512, 512],
    kernel_sizes: List[int] = [33, 39, 51, 63, 75],
    repeat_blocks: int = 1,
    decoder_input_channels: int = 1024,
    num_classes: int = 64):
    return nn.Sequential(QuartznetEncoder(feat_in, filters, kernel_sizes, repeat_blocks),
                         conv1d_decoder(decoder_input_channels, num_classes), nn.LogSoftmax(1))
